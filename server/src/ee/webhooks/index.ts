 ...
 */
const USER_AGENT = 'FormFlow-Webhook/1.0';
...
const webhookService = ({ strapi }: { strapi: Core.Strapi }) => ({
  ...
  async trigger(
    ...
    if (!config.url) {
      strapi.log.warn('[FormFlow] Webhook skipped: no URL configured');
    ...
    if (!this.isValidUrl(config.url)) {
      strapi.log.warn(`[FormFlow] Webhook skipped: invalid URL "${config.url}"`);
  ...
  async attemptDelivery(
    ...
    try {
      ...
      if (response.ok) {
        strapi.log.info(
          `[FormFlow] Webhook delivered: ${config.url} - Event: ${event} - ` +
      ...
      strapi.log.warn(
        `[FormFlow] Webhook returned error: ${config.url} - ${errorMessage} - ` +
      ...
      strapi.log.warn(
        `[FormFlow] Webhook attempt failed: ${config.url} - ${errorMessage} - ` +
  ...
  async triggerWithRetry(
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
      ...
      strapi.log.info(
        `[FormFlow] Webhook retry scheduled: ${config.url} - ` +
    ...
    strapi.log.error(
      `[FormFlow] Webhook delivery exhausted: ${config.url} - ` +
  ...
  dispatchWithRetry(
    ...
    if (inflightRetries >= MAX_INFLIGHT_RETRIES) {
      strapi.log.warn(
        `[FormFlow] Webhook retry capacity reached (${MAX_INFLIGHT_RETRIES} in flight); ` +
    ...
    void this.triggerWithRetry(config, event, form, submission, data)
      .catch((error: unknown) => {
        ...
        strapi.log.error(`[FormFlow] Webhook retry loop crashed: ${config.url} - ${message}`);
      })
      .finally(() => {
        inflightRetries -= 1;
      });
  ...
  async triggerAll(
    ...
    const activeWebhooks = webhooks.filter((w) => {
      ...
      if (!this.isValidUrl(w.url)) {
        strapi.log.warn(`[FormFlow] Webhook skipped: invalid URL "${w.url}"`);
        return false;
      }
